...
const paintTraining=step=>{const total=Object.values(step.totals).reduce((a,b)=>a+b,0);$('train-progress').value=step.index/step.total;$('train-status').textContent=`Round ${step.round+1} · ${step.index.toLocaleString()} / ${step.total.toLocaleString()} reachable positions · ${step.model.widths.join(' → ')}`;if(step.index===step.total){chartHistory.push(Object.fromEntries(Object.entries(step.totals).map(([name,count])=>[name,100*count/total])));drawChart()}};
...
$('train-reset').onclick=()=>{playing=false;trainer=null;chartHistory=[];$('train-progress').value=0;$('train-status').textContent='Weights reset. Change layers/nodes, then Start.';drawChart()};
drawChart();
render();
