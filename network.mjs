...
export function drawNetwork(canvas,history=''){
 ...
 ctx.font='12px ui-monospace,monospace';ctx.textAlign='center';ctx.fillStyle='#a6afa4';for(const col of cols)ctx.fillText(col.name,col.x,22);for(let ci=0;ci<cols.length;ci++){const col=cols[ci];for(let i=0;i<col.n;i++){const p=pos(col,i),selected=ci===3&&expertIndexes.includes(i);ctx.fillStyle=selected?'#d1ef65':ci===4?'#f6a65c':'#79cbe5';ctx.beginPath();ctx.arc(p.x,p.y,selected?7:ci===0?5:3,0,Math.PI*2);ctx.fill();if(ci===0){ctx.fillStyle='#f2f4ea';ctx.textAlign='right';ctx.fillText(String.fromCharCode(97+i),p.x-10,p.y+4)}if(ci===3){ctx.fillStyle=selected?'#d1ef65':'#a6afa4';ctx.textAlign='left';ctx.fillText(LABEL[EXPERTS[i]],p.x+10,p.y+4)}}}
