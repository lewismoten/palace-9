...
import { createModel } from './training.mjs';
import { routeTimeline, selectedExpert, activeExperts, hitTestNetwork, signedWeightColor, strongestRoutes } from './network.mjs';

assert.equal(selectedExpert(''), 'expert-1');
assert.equal(selectedExpert('aebd'), 'expert-1');
assert.deepEqual(routeTimeline('aebd').map(row => row.expert), ['expert-1','expert-1','expert-1','expert-1','expert-1']);
assert.deepEqual(activeExperts('aebd'),['expert-1','expert-2']);
const learnedModel=createModel({layers:2,nodes:8,experts:3,expertNodes:5});
const learnedRoute=activeExperts('aebd',learnedModel);
assert.equal(learnedRoute.length,2);
assert.ok(learnedRoute.every(key=>key.startsWith('expert-')));
